...
(function ($, window, document, undefined) {
    ...
    BaseTab.prototype.template = {
    	div_box:'<div class="content-tabs"></div>',
    	div_content_box: '<div class="page-tabs J_menuTabs"></div>',
    	btn_arrow_left: '<button class="roll-nav roll-left J_tabLeft"><i class="icon-leftslip"></i></button>',
    	btn_arrow_right: '<button class="roll-nav roll-right J_tabRight"><i class="icon-rightslip"></i></button>',
        ul_nav: '<ul id="myTab"  class="nav nav-tabs page-tabs-content"></ul>',
        ul_li: '<li class="J_menuTab"><a href="#{0}" target="{2}" data-toggle="tab"><span>{1}</span></a></li>',
        ul_li_close: '<i class="icon-close closeable" title="关闭" style="line-height: 1;"></i>',
        div_content: '<div class="tab-content"></div>',
        div_content_panel: '<div class="tab-pane fade" id="{0}"></div>',
        div_footer: '<div class="tab-footer">Copyright © 天际健康医疗科技有限公司</div>'
    }
    ...
    BaseTab.prototype.builder = function (data) {
        ...
        var div_box = $(this.template.div_box);
        var div_content_box = $(this.template.div_content_box);
        var btn_arrow_left = $(this.template.btn_arrow_left);
        var btn_arrow_right = $(this.template.btn_arrow_right);
        var div_content = $(this.template.div_content);
		var div_footer = $(this.template.div_footer);
        ...
        div_content_box.append(ul_nav);
        div_box.append(btn_arrow_left);
        div_box.append(div_content_box);
        div_box.append(btn_arrow_right);

        this.$element.append(div_box);
        this.$element.append(div_content);
		this.$element.append(div_footer);
        this.loadData();
    ...
    BaseTab.prototype.addTab=function (obj) {
        ...
        if(obj.closeable){
            this.$element.find(".nav-tabs li a[href='#" + obj.id + "'] i.closeable").click(function () {
                var href = $(this).parents("a").attr("href").substring(1);
                var temp= self.getCurrentTabId();
                var index = $("#myTab li").index($(this).parents("li"));//获得活动页的位置
                var index2=$('#myTab').children('li').length-1;
                $(this).parents("li").remove();
                $("#" + href).remove();
                if(temp==href){
                	if(index!=index2){
                		...
                		self.$element.find(".nav-tabs li:eq("+(index-1)+") a").tab("show");
                	}
                }
                var marginLeftVal = Math.abs(parseInt($('.page-tabs-content').css('margin-left')));
                var conWidth = $(".page-tabs-content").outerWidth();
                if(marginLeftVal == conWidth){
   ...
   //获取活动页的target
    BaseTab.prototype.getCurrentTabTarget=function () {
        var target=this.$element.find(".nav-tabs li.active a").attr("target");
        return target;
    }
